...
Page({
  ...
  onLoad(params = {}) {
    ...
    //let params = getApp().globalData.tripfinder_option
  ...
  onShareAppMessage: function () {
    let title = '我在搜索直升机滑雪行程，一起看看吧！'

    return {
      title: title,
      path: '/pages/tripfinder/index'
    }
  },
      ...
      if (n.checked) {
        if (n.value === 'start_date') {
          ...
          n.children.filter((n) => n.selected).forEach((n) => {
            ...
            } else if (n.value === 'lodge') {
              const selected = n.children.filter((n) => n.checked).map((n) => n.value).join(',')
              params.lodge = selected
  ...
  */
  ...
  /*
  onPullDownRefresh() { //下拉刷新功能，需要在index.json里添加： "enablePullDownRefresh": true
